...
\begin{abstract}
\begin{justify}
This study examines how the choice of linear system solver affects the practical behavior of Alternating Least Squares (ALS) for collaborative filtering. Using the MovieLens 100K dataset, we evaluate five solver strategies inside the ALS update step: explicit matrix inversion, Gaussian elimination with partial pivoting, LU decomposition, QR decomposition, and Cholesky decomposition. The investigation combines exploratory data analysis, model training, and solver-level diagnostics under a fixed experimental setup (latent factors $K=10$, regularization $\lambda=10$, and 30 epochs).

The exploratory analysis confirms that the rating distribution is generally concentrated toward higher scores and that user-item interactions are sparse but structurally consistent for matrix factorization. A filtered version of the dataset is also considered by removing extreme user and item average-rating profiles; this preserves central tendency while slightly reducing variability.

For solver comparison, three objectives are considered: computational efficiency, numerical stability, and recommendation quality. Results show that train and test RMSE trajectories are effectively identical across correctly implemented solvers, indicating that solver choice does not materially change recommendation quality on this benchmark. The primary practical difference is runtime. Numerical behavior is interpreted through residual norms $\|Ax-b\|$ for solver precision, while condition number is treated as a property of the ALS subproblem matrix rather than a solver-specific metric. Overall, the findings support selecting solvers primarily by efficiency for well-conditioned ALS subproblems, while favoring numerically robust factorizations in more ill-conditioned settings.
...
One of the most effective approaches to this problem is \textbf{matrix factorization}. The intuition is that user preferences and item characteristics can be described by a small number of underlying \textit{latent factors}, hidden concepts such as genre, tone, or style that are not explicitly labeled but can be inferred from rating patterns. For instance, a user who consistently rates action movies highly implicitly reveals something about their taste, even if that preference is never explicitly stated.
...
Finding \( U \) and \( V \) that simultaneously minimize \( \mathcal{L}(U, V) \) is difficult because the two unknowns are multiplied together, making the problem non-convex. However, there is creativite solution, if we temporarily fix one matrix and only optimize the other, the problem simplifies dramatically into a standard least-squares problem, which has a clean closed-form solution. \textbf{Altrrnating Least Squares (ALS)} does exactly this by repeatedly alternates between optimizing \( U \) with \( V \) held fixed, and optimizing \( V \) with \( U \) held fixed, until the solution converges \parencite{zhou2008large}.
...
Figure \ref{fig:rating-dist} shows the distribution is skewed to the higher ratings. The median is 4, and the mean is 3.53, indicating that users tend to give higher ratings more frequently than lower ones. 
...
Figure \ref{fig:avg-rating-user} shows the distribution of average ratings per user. This shows more granuality compared to Figure \ref{fig:rating-dist} and shows that some users have a tendency to give higher ratings than others.
